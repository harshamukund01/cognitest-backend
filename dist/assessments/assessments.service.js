...
let AssessmentsService = class AssessmentsService {
    ...
    async create(userId, data) {
        ...
        let visuospatialScore = null;
        let dailyLivingScore = null;
        let caregiverScore = null;
        let moodScore = null;
        if (!data.details)
            data.details = {};
        ...
        if (type === 'MEMORY') {
            ...
            const words = transcript.split(/\s+/).map((w) => w.toLowerCase().replace(/[^a-z]/g, '')).filter((w) => w.length > 0);
    ...
    async getReportPdfFile(reportId, userId) {
        const report = await this.prisma.report.findUnique({
            where: { id: reportId },
        });
        if (!report || report.userId !== userId) {
            throw new common_1.BadRequestException('Report not found or access denied');
        }
        const pdfFile = path.join(this.reportsDir, `${reportId}.pdf`);
        if (!fs.existsSync(pdfFile)) {
            throw new common_1.BadRequestException('PDF file is not compiled on disk');
        }
        return pdfFile;
    }
    async getDashboardStatus(userId) {
        ...
        const recentActivities = assessments.slice(0, 5).map(ast => {
            ...
            if (ast.type === 'MEMORY')
                title = 'Memory Sprint Test';
            else if (ast.type === 'LOGIC')
                title = 'Pattern Logic Test';
            else if (ast.type === 'SPEED')
                title = 'Reaction Speed Test';
            else if (ast.type === 'LANGUAGE')
                title = 'Verbal Language Test';
            return {
                id: ast.id,
                title,
                score: ast.score,
    ...
    async getHistoricalTrends(userId) {
    ...
    async calculateOverallScore(userId) {
        ...
        if (orientationScore !== null) {
            sum += orientationScore;
            count++;
        }
        ...
        if (visuospatialScore !== null) {
            sum += visuospatialScore;
            count++;
        }
        if (dailyLivingScore !== null) {
            sum += dailyLivingScore;
            count++;
        }
    ...
    generateClinicalRecommendations(score, type) {
        const list = [];
        if (score >= 85) {
        ...
        if (type === 'MEMORY')
            list.push('Use structured mnemonic associations during memory recall tasks.');
        if (type === 'LOGIC')
            list.push('Practice visual grid mapping templates to boost logical reasoning.');
        return list;
    }
    generateReportPdf(userId, reportId, summary, recommendations, latestAssessment) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                doc.fontSize(9)
                    .font('Helvetica')
                    .fillColor('#555555');
                const formatScore = (val) => val !== undefined && val !== null && val > 0 ? `${Math.round(val)}/100` : 'Not Completed';
    ...
    async submitPreScreening(userId, answers) {
        let oftenCount = 0;
    ...
    async submitMeditationSession(userId, data) {
